...
(function () {
  ...
  function deepBuildPrompt(prompt) {
    return isSpaceVehiclePrompt(prompt) || /aircraft|jet|robot|vehicle|factory|powerplant|engine|city|campus|bridge/.test((prompt||'').toLowerCase());
  }
  ...
  async function generate(rawPrompt) {
    ...
    showStatus(isSpaceVehiclePrompt(rawPrompt) ? 'Aerospace kernel built Starship Mk-I · deep engineering pass 1/3…' : 'Instant draft ready · researching real-world dimensions…', true);
    ...
    var contract = isSpaceVehiclePrompt(rawPrompt) ? '\n\nAEROSPACE ASSEMBLY CONTRACT: Produce at least 32 named objects and multiple primitive types. Include pressure hull, ogive nose, 7-engine cluster, nozzles, heat-shield tiles, weld rings, 4 independent flaps/fins, cockpit windows, RCS pods, avionics, landing legs and launch infrastructure. A single cone/cylinder is a failed design. Ground is Y=0; vehicle is vertical +Y.' : '';
    ...
    if (spec) {
      var candidateScore=sceneQuality(spec,rawPrompt), threshold=isSpaceVehiclePrompt(rawPrompt)?baselineScore*.72:1;
      if (candidateScore >= threshold) {
        ...
        showStatus('Weak AI proposal rejected · engineering-grade Starship preserved.', false);
        toast('Quality gate protected the '+baseline.objects.length+'-part aerospace model');
        setTimeout(hideStatus,3600);
      }
    } else {
      showStatus(isSpaceVehiclePrompt(rawPrompt) ? 'Cloud refinement unavailable · '+baseline.objects.length+'-part aerospace model preserved.' : 'Instant model kept — refinement unavailable ('+(err&&err.message||'offline')+').', false);
