...
RESET_SOURCES   = [r.strip() for r in os.environ.get("OMNI_RESET_SOURCES", "").split(",") if r.strip()]
...
def normalize(row: Dict[str, Any], hint: str) -> Optional[Dict[str, Any]]:
    ...
    if _DBG[0] < 12:
        _DBG[0] += 1
        prev = {k: (str(v)[:80] if v not in (None, "") else repr(v)) for k, v in list(row.items())[:10]}
